...
- **Personagens:** cada agente é representado por um **robô uniforme** (mesmo chassi) no seu módulo, com **micro-animações CSS** (idle flutua; trabalhando se inclina ao console). Gera-se **1 robô (2 poses)** com accent ciano; a cor por agente é aplicada por **código** (`hue-rotate`) — sem regenerar por agente. Movimento por CSS sobre sprite único (sem frames de IA); **sem perambular por corredores**.
...
> **Estado atual (jun/2026):** Fatias 0–2 concluídas. App roda local com nave FTL, 5 agentes em streaming, ARIA com busca real. Próxima: Fatia 3.
